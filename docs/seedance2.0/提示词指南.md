Seedance 2.0 系列模型原生支持音频与视频联合生成，拥有卓越的语义理解与多模态交互能力。本文介绍 Seedance 2.0 （同样适用于 Seedance 2.0 fast）的提示词使用方法和相关技巧，帮助您更高效地利用该模型生成符合需求的优质视频作品。
:::warning
...
## 1.1 文本指令的基础公式
Seedance 2.0 能深度遵循自然语言逻辑，因此您可以根据需求灵活组合以下元素。
...
# 02 文字生成
Seedance 2.0 支持在 T2V（文生视频）、I2V（图生视频）、R2V（参考生视频）、V2V（视频生视频）等场景下生成常用文字。
...
# 04 视频参考
Seedance 2.0 支持视频参考，使用时指代清楚生成内容和参考对象即可。
...
# 05 视频编辑
Seedance 2.0 支持视频编辑，支持增加、删除或修改元素，视频的向前或向后延长，以及轨道补齐。
使用过程中，如对视频顺序有要求，应**按顺序上传**，提示词中可使用`视频1`、`视频2`……`视频n`进行准确指代。
...
* Seedance 2.0 最多支持 3 段视频输入，总时长不得超过 15 秒。

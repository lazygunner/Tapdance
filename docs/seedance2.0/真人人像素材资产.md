...
# 使用前准备
要使用私域素材库的全部功能，您需购买高级创作权益说明包。私域虚拟素材库与真人人像素材库共享容量。具体信息请参考 [Seedance 2.0 高级创作权益包购买说明](/docs/82379/2377608)**。**
<span id="7d7aed21"></span>
# 使用流程概述
真人人像素材库使用流程如下：
<div style="text-align: center"><img src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/3e53e94c7c5749788a0f7afeaf042717~tplv-goo7wpa0wc-image.image" width="5948px" /></div>


* [Step 1. 创建真人人像素材组（Asset Group）](/docs/82379/2333589#d9a7d853)
...
* [Step 2. 上传/管理素材](/docs/82379/2333589#5c74319f)（创建 Assets）
   * 素材资产（Assets）入库时，系统会将上传图像与真人认证采集的基准图像进行面部特征一致性比对，比对通过后方可入库。
   * 可使用Assets API 检索素材 ID、更新素材信息或删除素材。
* [Step 3. 使用人像素材生成视频](/docs/82379/2333589#ee62e0ae)
...
* 每个上传的素材资产需经过预处理，可轮询调用 **GetAsset** 接口查询素材状态（对应参数为 **Status）**，仅当状态变为 `Active` 后，该素材资产方可用于后续推理使用；若状态为 `Failed` 则表示处理失败，无法用于后续推理使用。**详情可参考：**[私域真人人像素材资产使用指南](/docs/82379/2333589)。
:::
...
| | | \
|CreateAsset |不同权益用户限流不同，具体请参见 [Seedance 2.0 高级创作权益包购买说明](/docs/82379/2377608)。 |
| | | \
...
* 详细后缀参数：
   * bytedToken：本次认证的唯一凭证标识，用来在`GetVisualValidateResult` 获取本次创建的 Group ID。
   * resultCode：
      * **当 resultCode 为** **`10000` 时，检测成功**。
      * 更多错误码信息请参考真人认证相关[错误码](https://www.volcengine.com/docs/86081/1660367?lang=zh)文档。
   * algorithmBaseRespCode：服务端子错误码。建议resultCode为服务端错误码时，再检查该字段对应的错误类型。具体请参考真人认证相关[错误码](https://www.volcengine.com/docs/86081/1660367?lang=zh)文档。
...
<BytedReactXgplayer config={{ url: 'https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/a7e679bff3e44d0fb4b252a0507bd3de~tplv-goo7wpa0wc-image.image', poster: 'https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/a7e679bff3e44d0fb4b252a0507bd3de~tplv-goo7wpa0wc-video-poster.jpeg' }} ></BytedReactXgplayer>
...
<div style="text-align: center"><img src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/8dd278ff06654fb587c00a494c17832b~tplv-goo7wpa0wc-image.image" width="306px" /></div>
...
<div style="text-align: center"><img src="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/245f1f5c008b42ec84a5c91349b9c5af~tplv-goo7wpa0wc-image.image" width="1080px" /></div>
...
:::tip
**注意**：要获取完整的 API 参数、限流等信息，请查看[私域真人人像库 API 参考文档](/docs/82379/2333602)。
:::
```Go
package main
...
:::tip
获取完整的 API 参考文档，请查看[私域虚拟人像库 API 参考文档](/docs/82379/2333601)。
:::
```Go
package main
...
:::tip
**注意**：要获取完整的 API 参考文档，请查看[私域真人人像库 API 参考文档](/docs/82379/2333602)。
:::
```Go
package main
...
### 更新/删除素材或素材组
具体信息请参考：[私域真人人像库 API 参考文档](/docs/82379/2333602)
...
:::tip
此处建议使用 CallbackURL 后缀参数获取真人验证结果；当 **resultCode** 为 **10000**（真人认证通过）时，可使用`GetVisualValidateResult`获取本次真人认证创建的 Group ID。
:::
...
示例代码：
<Attachment link="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/a6717347bf404eaab92d4f115f7b4e30~tplv-goo7wpa0wc-image.image" name="Generate_H5&Get_Group_ID.go" ></Attachment>
返回示例：
```Plain Text
...
示例代码：
<Attachment link="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/268a0560a533491bb10df1eeeffa979e~tplv-goo7wpa0wc-image.image" name="Upload_Asset&_Get_Asset_Info.go" ></Attachment>
查询结果示意如下：
```JSON
asset status: Active
...
## 其他编程语言示例代码
<Attachment link="https://p9-arcosite.byteimg.com/tos-cn-i-goo7wpa0wc/de915358f2114603a4d425195cca8835~tplv-goo7wpa0wc-image.image" name="demo.zip" ></Attachment>
:::tip
